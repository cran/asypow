...
C     .. Executable Statements ..
   ...
   50 CONTINUE
   60 alngam = revlpl(scoefn,9,xx-2.0D0)/revlpl(scoefd,4,xx-2.0D0)
C
C
...
C
C
  100 alngam = revlpl(coef,5,1.0D0/ (xx**2))/xx
...
C                  search bound
C                3 if there was an unexpected error
C                    INTEGER STATUS
C
...
C
          x = 5.0D0
          CALL stinvr(0.0D0,inf,0.5D0,0.5D0,5.0D0,atol,tol)
          status = 0
          CALL invr(status,x,fx,qleft,qhi)
...
C
          df = 5.0D0
          CALL stinvr(zero,inf,0.5D0,0.5D0,5.0D0,atol,tol)
          status = 0
          CALL invr(status,df,fx,qleft,qhi)
...
C
          pnonc = 5.0D0
          CALL stinvr(0.0D0,tent4,0.5D0,0.5D0,5.0D0,atol,tol)
          status = 0
          CALL invr(status,pnonc,fx,qleft,qhi)
...
C
  430 ans = 2.0D0
      ...
      END
      SUBROUTINE invr(status,x,fx,qleft,qhi)
C**********************************************************************
C
C     SUBROUTINE INVR(STATUS, X, FX, QLEFT, QHI)
...
C
C                 If SMALL, X, BIG not monotone in INVR it returns
C                 status -2
C                         INTEGER STATUS
C
...
C     GET-FUNCTION-VALUE
  ...
  230 CONTINUE
  240 CALL stzror(xlb,xub,abstol,reltol)
C++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
...
C++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  260 CALL zror(status,x,fx,xlo,xhi,qdum1,qdum2)
...
C     GET-FUNCTION-VALUE
  ...
  290 x = xlo
C      IF (QRZERO(F,Y,XLB,XUB,ABSTOL,RELTOL)) CONTINUE
C      X = XLB
      status = 0
      RETURN

      ENTRY stinvr(zsmall,zbig,zabsst,zrelst,zstpmu,zabsto,zrelto)
C**********************************************************************
C
C      SUBROUTINE STINVR( SMALL, BIG, ABSSTP, RELSTP, STPMUL,
C     +                   ABSTOL, RELTOL )
C                    SeT INVerse finder - Reverse Communication
C
C
C                              Function
C
...
C     TO GET-FUNCTION-VALUE
  ...
  310 CONTINUE
      ...
      END
      DOUBLE PRECISION FUNCTION revlpl(a,n,x)
C
C**********************************************************************
C
C     DOUBLE PRECISION FUNCTION REVLPL(A,N,X)
...
C
      ...
      END
      SUBROUTINE zror(status,x,fx,xlo,xhi,qleft,qhi)
C**********************************************************************
C
C     SUBROUTINE ZROR(STATUS, X, FX, XLO, XHI, QLEFT, QHI)
C          ZeRo of a function -- Reverse Communication
C
C
C                              Function
C
...
C     GET-FUNCTION-VALUE
  ...
  260 RETURN
      ...
      ENTRY stzror(zxlo,zxhi,zabstl,zreltl)
C**********************************************************************
C
C     SUBROUTINE STZROR( XLO, XHI, ABSTOL, RELTOL )
C          SeT ZeRO finder - Reverse communication version
C
C
C                              Function
C
